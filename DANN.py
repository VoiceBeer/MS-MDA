...
LastEditTime: 2021-02-22 07:33:25
'''
# standard
import argparse
import torch
...
class DANNNet():
    def __init__(self, model=models.DANN(), source_loader=0, target_loader=0, batch_size=64, iteration=10000, lr=0.001, momentum=0.9, log_interval=10):
        self.model = model
...
def cross_subject(data, label, session_id, category_number, batch_size, iteration, lr, momentum, log_interval):
    ...
    target_loader = torch.utils.data.DataLoader(dataset=utils.CustomDataset(target_data, target_label),
                                                            ...
                                                            shuffle=True, 
                                                            drop_last=True)
    model = DANNNet(model=models.DANN(pretrained=False, number_of_category=category_number),
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='DANN parameters')
    parser.add_argument('--dataset', type=str, default='seed3', help='the dataset used for DANN, "seed3" or "seed4"')
    ...
    # data preparation
    print('Model name: DANN. Dataset name: ', dataset_name)
